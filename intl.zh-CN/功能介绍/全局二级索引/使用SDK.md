...
**说明：** 您也可以使用CreateTable接口在创建数据表的同时创建一个或者多个索引表，详情请参见[创建数据表](/intl.zh-CN/SDK 参考/Java SDK/表/创建数据表.md)。
...
-   单行读取索引表中数据
    ...
    详情请参见[单行数据操作](/intl.zh-CN/SDK 参考/Java SDK/单行数据操作.md)。
...
-   范围读索引表中数据
    ...
    详情请参见[多行数据操作](/intl.zh-CN/SDK 参考/Java SDK/多行数据操作.md)。
        ...
        private static void scanFromIndex(SyncClient client) {
            ...
            while (true) {
                ...
                for (Row row : getRangeResponse.getRows()) {
                    ...
                    criteria.addColumnsToGet(DEFINED_COL_NAME3); //设置读取数据表的DEFINED_COL_NAME3列。
                    //设置读取最新版本。
