...
|GetRow|对局部事务范围内的数据进行读写操作。具体操作，请参见[单行数据操作](/intl.zh-CN/功能介绍/Wide column模型/基础数据操作/单行数据操作.md)和[多行数据操作](/intl.zh-CN/功能介绍/Wide column模型/基础数据操作/多行数据操作.md)。**说明：** 当前局部事务范围在一个分区键值内，分区键的更多信息，请参见[主键和属性](/intl.zh-CN/功能介绍/Wide column模型/主键和属性.md)。 |
|PutRow|
|UpdateRow|
|DeleteRow|
|BatchWriteRow|
