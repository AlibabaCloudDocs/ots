...
-   When multiple rows are written, part of rows may fail to be written. If the operation fails for part of rows, the system does not return exceptions. However, information about the index and error messages of the failed rows is contained in BatchWriteRowResponse. Therefore, when you call the BatchWriteRow operation, you must check the returned row to determine whether the status of each row is successful. If you do not check the returned value, some operations may fail to be performed.
...
-   Operations
     ...
     * Note that if the BatchWriteRow operation fails for part of rows, the system does not return an exception. However, information about the failed rows is contained in $response. For more information, see the sample response to the BatchWriteRow operation.
     * @api
...
-   Response parameters
    ...
    |is\_ok|Indicates whether the operation for this row was successful.     -   A value of true indicates that the row is written. In this case, ignore errors.
    ...
    |consumed|The value of capacity units consumed by this operation. capacity\_unit: the number of read and write CUs consumed.
    ...
    |attribute\_columns|The values of attribute columns. This parameter name is consistent with that in the request. At present, this parameter value is empty.|
...
-   Examples
    ...
    The following code provides an example on how to write 30 rows of data to three data tables at a time. Each table is written with 10 rows.
...
-   Operations
     ...
     * Note that if the BatchGetRow operation fails for part of rows, the system does not return an exception. However, information about the failed rows is contained in $response. For more information, see the sample response to the BatchGetRow operation.
     * @api
     * @param [] $request The request parameters.
     * @return [] The response.
     * @throws OTSClientException The exception that occurs when a parameter error occurs or the Tablestore server returns a verification error.
...
-   Response parameters
    ...
    |---------|-----------|
    |table\_name|The name of the table.|
    |is\_ok|Indicates whether the operation for this row was successful.     -   A value of true indicates that the row is read. In this case, ignore errors.
...
A timestamps is a 64-bit integer, which indicate multiple versions of data.
    ...
    |next\_token|The starting position of a wide row in the next read operation. This function is unavailable.|
...
-   Examples
    ...
    The following code provides an example on how to read 30 rows of data from three tables at a time. Each table is written with 10 rows of data.
...
-   The number of rows scanned reaches 5,000.
...
-   Parameters
    ...
    Difference between GetRange and GetRow:
    ...
    -   inclusive\_start\_primary\_key specifies the start primary key column. If the row exists, the response contains this row.
    -   exclusive\_end\_primary\_key specifies the end primary key column. The response excludes the row no matter whether the row exists.
Rows in tables are sorted based on the primary key column values in ascending order. The range for reading is a left-closed and right-open interval. When data is read in the forward direction, all rows whose primary key column values are greater than or equal to the start primary key column value and smaller than the end primary key column value. |
    |exclusive\_end\_primary\_key|
    |direction|The direction for reading.    -   If DirectionConst::CONST\_FORWARD is set, the start primary key column value must be smaller than the end primary key column value. The rows are returned based on the primary key column values in ascending order.
...
Example: A table has two primary keys A and B where the value of A is smaller than that of B. If you set direction to forward, the rows whose primary key values are greater than or equal to the value of A and smaller than the value of B are returned in ascending order. If you set direction to backward, the rows whose primary key values are greater than or equal to the value of B and smaller than the value of A are returned in descending order. |
    ...
    |max\_versions|The maximum number of versions you want to read.**Note:** You must specify at least one of max\_version and time\_range.
...
-   Response parameters
    ...
    |attribute\_columns|The values of attribute columns.    -   Each item specifies the values in the following sequence: the attribute column name, attribute column value \(ColumnValue\), attribute column value type \(ColumnType\), and timestamp.

A timestamps is a 64-bit integer, which indicate multiple versions of data. Unit: milliseconds.
...
-   Examples
    ...
    ```
    // Search for rows whose PK0 value is from within [1, 4].
