...
This topic describes how to test the performance of incremental synchronization through Tunnel Service, and elaborates the test environment, tools, plan, indicators, results, and rules.
...
-   Pre-splitting tool
    ...
    The stress testing tool of Tablestore can automatically create and pre-split tables based on the configured table names and the number of partitions.

-   Rate statistics tool

    Tablestore SDK for Java can collect statistics of the consumption rate of incremental data and the total number of consumed rows in real time. You can add the logic demonstrated in the following example to the callback to collect rate statistics.
...
-   Data processing tasks are automatically implemented in Tunnel Service. The tunnel client can be started by using the same TunnelId. For more information, see [Tunnel clients](/intl.en-US/Function Introduction/Tunnel service/Tunnel clients.md).
...
-   QPS and latency
    ...
    In the single-server 64-partition synchronization scenario, the gigabit NIC works at its full capacity, resulting in only 570,000 QPS. For more information, see the test details. The QPS in the double-server 64-partition synchronization scenario reaches 780,000, almost twice as much as the 420,000 QPS achieved in the single-server 32-partition synchronization scenario. In the double-server 128-partition synchronization scenario, the QPS reaches 1,000,000.
...
-   System resource consumption
    ...
    In the single-server single-partition synchronization scenario, 0.25 single-core CPUs are used. In the double-server 128-partition synchronization scenario, only 10.2 single-core CPUs are used when the QPS reaches 1,000,000. The memory usage increases linearly with the number of partitions when it is less than 32. When more partitions \(32 or 64 partitions\) need to be processed, the memory usage remains stable around 5.3 GB on each server.
...
-   Single-server 64-partition synchronization: 570,000 QPS, with the gigabit NIC working at its full capacity.
    ...
    -   Memory usage: approximately 8.4% of the total physical memory, or 5.376 GB. \(Each test server provides 64 GB of physical memory.\)
...
-   Double-server 64-partition synchronization: 780,000 QPS.
    ...
    -   Memory usage: approximately 8.2% of the total physical memory, or 10.5 GB. \(Each test server provides 64 GB of physical memory.\)
