...
1. 在表格存储侧创建数据表或全局二级索引。
   ...
   1. 创建表格存储的数据表，详情请参见[概述](/cn.zh-CN/快速入门/概述.md)。
      ...
      ![fig_00a](//static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/7142343061/p175968.png)
      
   
   2. （可选）在数据表上创建全局二级索引，详情请参见[使用SDK](/cn.zh-CN/功能介绍/全局二级索引/使用SDK.md)。
      ...
      **说明**
      ...
      ![fig_00b](//static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/7142343061/p175971.png)
...
1. 在表格存储侧创建数据表和多元索引。
   ...
   1. 创建数据表，详情请参见[概述](/cn.zh-CN/快速入门/概述.md)。

      本示例中数据表名称为geo_table，主键列为pk1（String类型），属性列分别为val_keyword1（String类型）、val_keyword2（String类型）、val_keyword3（String类型）、val_bool（Boolean类型）、val_double（Double类型）、val_long1（Long类型）、val_long2（LongG类型）、val_text（String类型）和val_geo（String类型），数据条数为208912382，数据样例如下图所示。

      ![fig_002](//static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/7142343061/p175963.png)
      
   
   2. 在数据表上创建多元索引，详情请参见[创建及使用多元索引](/cn.zh-CN/快速入门/创建及使用多元索引.md)。
      ...
      **说明**
      ...
      ![fig_001](//static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/7142343061/p175962.png)

      创建多元索引后，多元索引会自动开始同步数据表中的数据，待多元索引进入增量状态时，表示多元索引完成构建。

      ![fig_searchindex_batch_list](//static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/8733279951/p164617.png)
...
2. 在EMR集群侧创建Spark外表。
   ...
   2. 创建Spark外表同时连接多元索引。
      ...
      * 参数
        ...
        | push.down.range.long   | 与Long类型的列做大小（\>=、\>、\<、\<=）比较的谓词是否下推，详情请参见[批计算谓词下推配置](/cn.zh-CN/计算与分析/Spark/SparkSQL/细则剖析/批计算谓词下推配置.md)。 类型为Boolean，默认值为true，表示与Long类型的列做大小比较的谓词下推。 设置为false时，表示与Long类型的列做大小比较的谓词不下推。详情请参见。 |
        | push.down.range.string | 与String类型的列做大小（\>=、\>、\<、\<=）比较的谓词是否下推，详情请参见[批计算谓词下推配置](/cn.zh-CN/计算与分析/Spark/SparkSQL/细则剖析/批计算谓词下推配置.md)。 类型为Boolean，默认值为true，表示与String类型的列做大小比较的谓词下推。 设置为false时，表示与String类型的列做大小比较的谓词不下推。 |
